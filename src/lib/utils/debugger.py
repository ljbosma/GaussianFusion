...
class Debugger(object):
  ...
  #             isinstance(pt1[0], (int, float, np.integer, np.floating)) and \
  #             isinstance(pt1[1], (int, float, np.integer, np.floating)) and \
  #             isinstance(pt2[0], (int, float, np.integer, np.floating)) and \
  #             isinstance(pt2[1], (int, float, np.integer, np.floating)):

  #               pt1 = (int(pt1[0]), int(pt1[1]))
  #               pt2 = (int(pt2[0]), int(pt2[1]))
  #               cv2.line(bird_view, pt1, pt2, lc, t, lineType=cv2.LINE_AA)
  #           else:
  #               print(f"[Warning] Invalid point(s) for cv2.line: pt1={pt1}, pt2={pt2}")
