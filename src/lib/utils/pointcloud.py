...
def gaussian_fused_point_only_bbox(pc_dep_roi, bbox, dep, dist_thresh, opt):
    ...
    coords = torch.stack([x.flatten(), y.flatten()], dim=1).float()  # shape: [N, 2]
    values = pc_depth.flatten()

    # Filter valid depth points
    valid = (values > 0) & \
            (values < dep + dist_thresh) & \
            (values > max(0, dep - dist_thresh))
    coords = coords[valid]
    values = values[valid]

    if coords.shape[0] < 2:
        return values[0] if values.numel() > 0 else torch.tensor(0.0, device=pc_depth.device)

    # 1. Center of the frustum in image plane (bbox center)
    mean = torch.tensor([(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], device=coords.device)

    # 2. Covariance matrix based on bbox size
    w = bbox[2] - bbox[0]
    h = bbox[3] - bbox[1]
    scale = opt.gaussian_cov_scale if hasattr(opt, "gaussian_cov_scale") else 0.1
    cov = torch.tensor([[scale * w ** 2, 0.0], [0.0, scale * h ** 2]], device=coords.device)
    ...
    weighted_depth = (values * weights).sum()
...
def gaussian_fused_point_dep_and_xbbox(pc_dep, bbox, dep, dist_thresh, nonzero_inds, opt):
    ...
    valid = (values < dep + dist_thresh) & (values > max(0, dep - dist_thresh))
    if valid.sum() < 1:
        return torch.tensor(0.0, device=pc_dep.device)

    values = values[valid]
    xs = w_idxs[valid].float()  # x-position within ROI
    coords = torch.stack([xs, values], dim=1)  # [N, 2] -> (x, depth)

    # --- Gaussian parameters ---
    mean_x = (bbox[0] + bbox[2]) / 2
    mean_d = dep
    mean = torch.tensor([mean_x, mean_d], device=pc_dep.device)
...
def pc_dep_to_hm_torch(pc_hm, pc_dep, dep, bbox, dist_thresh, opt, expand_x=0.0):
    if isinstance(dep, list) and len(dep) > 0:
      dep = dep[0]
    ct = torch.tensor(
      [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], dtype=torch.float32)
    w = bbox[2] - bbox[0]
    expand_pixels = w * min((expand_x + opt.dynamicFrustumExpansionRatio * dep.item()**2), 2)
    bbox = bbox.clone()
    ...
    #   nonzero_pc_dep = torch.exp(-pc_dep[nonzero_inds])
      ...
      if len(pc_dep_match) > 0:
        dist = gaussian_fused_point_dep_and_xbbox(pc_dep, bbox.clone(), dep, dist_thresh, nonzero_inds, opt)
